  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const fruitProducts = Array.isArray(data)
          ? data.filter((p: Product) => p.category === "fruit").sort((a, b) => Number(a.id) - Number(b.id))
          : [];
    ...
    <main className="min-h-screen bg-[#FFFBF0]">
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="inline-block bg-[#FFD1DC] px-6 py-2 rounded-full mb-6">
          <h1 className="text-2xl font-bold text-[#4A3728]">Fruit Yogurts</h1>
        </div>
        {loading ? (
          <div>Loading...</div>
        ) : error ? (
